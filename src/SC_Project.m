...
for i = 1:100
    ...
    for j = 2:ny-1 
        %F(j,2) =
        %sin(pi*(x(2)-ax)/(bx-ax)).*cos(pi*(2*(y(j)-ay)/(by-ay)+1)/2);
        %(old)
        %u(j,2) = (2*u(j,3)+u(j-1,2)+u(j+1,2)+F(j,2)*hy*hx)/4;
        %F(j,nxplus-1) =
        %sin(pi*(x(nxplus-1)-ax)/(bx-ax)).*cos(pi*(2*(y(j)-ay)/(by-ay)+1)/2);
        %(old)
        %u(j,nxplus-1) = (2*u(j,nxplus-2)+u(j-1,nxplus-1)+u(j+1,nxplus-1)+F(j,nxplus-1)*hy*hx)/4;
        u(:,1) = u(:,3);
        u(:,nxplus) = u(:,nxplus-2);
        for k = 2:nxplus-1      
